...
%  项目补充：机械原理
% ============================================================
\usepackage[table]{xcolor}   % \rowcolor 等表格着色
\usepackage{siunitx}
\usepackage{yhmath}
\usepackage{wallpaper}
\usepackage{xparse}

% 无编号 definition 环境（可选标题，行内绿色标签，支持跨页）
\RenewDocumentEnvironment{definition}{o}
    {\begin{tcolorbox}[lisiBox, colframe=LiSiDef]%
     \noindent{\bfseries\songti\color{LiSiDef}定义\IfNoValueF{#1}{（#1）}}\hspace{0.4em}\ignorespaces}
    {\end{tcolorbox}}

% 带色块标题的强调盒（保留原 mybox 接口）
\tcbset{
    mybox/.style = {
        enhanced, breakable,
        colback = white,
