...
\newtcolorbox{defBox}[3][]{commonbox,
    ...
    before title={\refstepcounter{theorem}},
    title={{#2}\ \thetheorem\ifstrempty{#3}{}{（#3）}}, #1}
% 机械原理使用文件末尾自定义的无编号 definition 环境，
% 此处不再定义带编号的 definition，避免重复定义冲突。
...
% 自定义无编号 definition 环境（可选参数为标题，主题化标题签，支持跨页）
\NewDocumentEnvironment{definition}{o}
  {\IfNoValueTF{#1}
    {\begin{tcolorbox}[breakable, commonbox, colback=defBg, colbacktitle=defColor, title={定义}]}
    {\begin{tcolorbox}[breakable, commonbox, colback=defBg, colbacktitle=defColor, title={定义：#1}]}}
  {\end{tcolorbox}}
