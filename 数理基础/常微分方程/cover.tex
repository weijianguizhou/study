\begin{titlepage}
	\definecolor{forestgreen}{RGB}{34,92,56}   % 森林绿主题色（与数理基础其余课程封面统一）
	\begin{tikzpicture}[remember picture, overlay]
		...
		% ---- 4. 右上：指数衰减振荡解曲线（二阶线性方程解 $x(t)=e^{-\alpha t}\sin\omega t$）----
		...
		% ---- 5. 左下：指数增长曲线（Malthus 模型 $N(t)=N_0 e^{rt}$）----
		\begin{scope}[shift={([xshift=0.9cm,yshift=0.9cm]current page.south west)}]
			...
			\node[anchor=south east, text=white, opacity=0.7, font=\footnotesize\itshape] at (-0.18,4.2) {$N(t)$};
		...
		\node[anchor=north, text=white, opacity=0.85, align=center] at (T) {
			{\sffamily\fontsize{13}{16}\selectfont
				\uppercase{Ordinary \ \ Differential \ \ Equations}}
		};

		% ---- 8. 主标题（两行，居中）----
		\node[anchor=center, align=center, text=white] at ([yshift=-8.0cm]current page.north) {%
			{\fontsize{44}{54}\sffamily\bfseries 常微分方程}\\[0.4cm]
		};
