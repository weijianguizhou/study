...
\begin{solution}
对应齐次方程 $t^2x'' - tx' + x = 0$，改写为 $x'' - \dfrac{1}{t}x' + \dfrac{1}{t^2}x = 0$。验证 $x = t$ 与 $x = 1$ 是解（$x'' = 0, x' = 1$ 代入得 $0 - t\cdot 1 + t = 0$），故基本解组为 $\{1, t\}$ $[?]$。常数变易，令 $x = c_1(t) + c_2(t)t$，得方程组 $\begin{cases} c'_1 + c'_2t = 0 \\ c'_2 = \dfrac{1}{t} \end{cases}$，解得 $c'_1 = -1, c'_2 = \dfrac{1}{t}$，积分 $c_1 = -t + \gamma_1, c_2 = \ln|t| + \gamma_2$，通解
...
\begin{solution}
特征方程 $\lambda^2 - 2\lambda - 3 = 0$，根 $\lambda = 3, -1$，齐次通解 $c_1e^{3t} + c_2e^{-t}$。$\lambda = 0$ 不是特征根，设特解 $\tilde{x} = At + B$，代入得 $A = -\frac{1}{3}, B = -\frac{1}{3}$ $[?]$，通解
...
\begin{solution}
特征方程 $(\lambda+1)^3 = 0$，$\lambda = -1$ 为三重根，齐次通解 $(c_1 + c_2t + c_3t^2)e^{-t}$。设特解 $\tilde{x} = t^3e^{-t}(At + B)$，代入得 $A = \frac{1}{24}, B = -\frac{5}{6}$ $[?]$，通解
...
\textbf{练习}：求 $x'' - 2x' - 3x = e^t + t^2 + 1$ 的通解 $\rightarrow$ 通解 $c_1e^{3t} + c_2e^{-t} - \frac{1}{4}e^t - \frac{1}{3}t^2 + \frac{4}{9}t - \frac{13}{27}$ $[?]$
...
\begin{solution}
设级数解 $y = \sum a_n x^n$，由初始条件 $a_0 = 0, a_1 = 1$。代入方程比较系数得递推关系：偶数项 $a_{2k} = 0$，奇数项 $a_{2k+1} = \dfrac{1}{(2k+1)!!}$ $[?]$，故
