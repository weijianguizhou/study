...
\subsection{总体与样本}
\begin{definition}[总体]
研究对象的全体。通常指研究对象的某项数量指标。组成总体的元素称为个体。从本质上讲，总体就是所研究的随机变量或随机变量的分布。
\end{definition}

\begin{definition}[样本（简单随机样本）]
来自总体的部分个体 $\mathbf{X}_1, \dots, \mathbf{X}_n$。如果满足：
\begin{enumerate}
...
称样本 $X_1, \cdots, X_n$ 的函数 $g(X_1, \cdots, X_n)$ 是总体 $X$ 的一个\textbf{统计量}，如果 $g(X_1, \cdots, X_n)$ \textbf{不含未知参数}。
\end{definition}

\begin{property}[几个常用的统计量]
\begin{enumerate}
...
\subsection{$\chi^2$ 分布}
\begin{definition}[$\chi^2$ 分布（构造）]
设 $X_1, \cdots, X_n$ 独立同分布于标准正态分布，即 $X_1, \cdots, X_n \overset{iid}{\sim} N(0,1)$，则定义统计量
\begin{equation}
	\chi^2 = \sum_{i=1}^{n} X_i^2 \sim \chi^2(n)
	\label{eq:抽样分布:卡方构造}
\end{equation}
称为\textbf{自由度为 $n$ 的 $\chi^2$ 分布}。其概率密度函数为：
\begin{equation}
...
\begin{tipbox}[笔记要点]
$\chi^2$ 分布本质上就是 $n$ 个独立的标准正态随机变量的平方和。分布是非对称的（右偏），取值恒为正。随着自由度 $n$ 的增大，曲线峰值向右移动，且图形逐渐趋向于对称（正态分布）。
\end{tipbox}
...
\begin{equation}
	...
	\label{eq:抽样分布:t构造}
\end{equation}
称 $t(n)$ 为\textbf{自由度为 $n$ 的 $t$ 分布}。其概率密度为：
\begin{equation}
	...
	\label{eq:抽样分布:t密度}
\end{equation}
其中 $\Gamma(\cdot)$ 是伽马函数（Gamma Function，欧拉第二积分），它是阶乘在实数域上的推广；$\left(1+\frac{t^2}{n}\right)^{-\frac{n+1}{2}}$ 是函数的主体部分，决定了 $t$ 分布呈钟形曲线，且比正态分布下降得更慢（即“厚尾”）。
\end{definition}

\begin{tipbox}[$t$ 分布的直观理解]
\begin{itemize}
...
\begin{keybox}[重点性质：分位点的对称性]
由于 $t$ 分布关于 $t=0$ 对称，因此
\begin{equation}
	t_{1-\alpha}(n) = -t_{\alpha}(n)
	\label{eq:抽样分布:t分位点对称性}
\end{equation}
示例：$t_{0.01}(20) = 2.5280$；$t_{0.99}(20) = t_{1-0.01}(20) = -t_{0.01}(20) = -2.5280$。
\end{keybox}

\subsection{$F$ 分布}
\begin{definition}[$F$ 分布（构造）]
$F$ 分布是通过两个独立的卡方（$\chi^2$）随机变量的比值构造而成的。设 $\eta_1$ 和 $\eta_2$ 是两个独立的卡方随机变量，其自由度分别为 $n_1$ 和 $n_2$：
\begin{itemize}
...
\begin{equation}
	...
	\label{eq:抽样分布:F构造}
\end{equation}
\textbf{自由度}：$n_1$ 称为\textbf{第一自由度}（分子自由度），$n_2$ 称为\textbf{第二自由度}（分母自由度）。其概率密度函数 $h(y)$ 如下（其中 $\Gamma$ 是伽马函数）：
\begin{equation}
...
\begin{definition}[$F$ 分布的分位点]
对于一个概率 $\alpha$（$0 < \alpha < 1$），如果存在一个正数 $F_{\alpha}(n_1, n_2)$，使得随机变量 $F$ 大于或等于该值的概率恰好是 $\alpha$，则称 $F_{\alpha}(n_1, n_2)$ 为 $F(n_1, n_2)$ 分布的上侧 $\alpha$ 分位点。
\begin{equation}
...
\begin{theorem}[标准化样本均值的分布（$U$ 统计量）]
当总体方差 $\sigma^2$ \textbf{已知}时，标准化后的样本均值服从标准正态分布 $N(0, 1)$。
\begin{equation}
...
\begin{theorem}[样本方差的分布（$\chi^2$ 统计量）]
将样本方差 $S^2$ 乘以适当的系数后，服从自由度为 $n-1$ 的 $\chi^2$ 分布（卡方分布）。
\begin{equation}
...
\begin{theorem}[$t$ 统计量的分布（小样本下的均值检验）]
当总体方差 $\sigma^2$ \textbf{未知}时，使用样本标准差 $S$ 替代 $\sigma$ 对 $\bar{X}$ 进行标准化，得到的统计量服从自由度为 $n-1$ 的 $t$ 分布。
\begin{equation}
