...
\begin{tcolorbox}[title=备注 (Remark), colframe=lightgray!60!black, colback=lightgray!10!white, sharp corners]
	一般来说，任意两个复数不能比较大小。
\end{tcolorbox}
...
\begin{tcolorbox}[title=定理：共轭虚根定理, colframe=blue!50!black, colback=blue!5!white]
	若 $z$ 是实系数多项式方程
	...
	又 $\because C_k = \bar{C}_k$（实数的共轭是其本身），以及 $\overline{z^n} = \bar{z}^n$，$\dots \bar{z}$ 满足原方程。
\end{tcolorbox}
...
\subsection{性质与证明}
\begin{tcolorbox}[title=复数的模与不等式性质, colframe=green!45!black, colback=green!5!white]
...
$\because y^2 \ge 0, x^2 \ge 0 \quad \therefore |z| \ge |x|, |z| \ge |y|$

$|z|^2 = x^2 + y^2$，$(|x| + |y|)^2 = x^2 + y^2 + 2|xy| \ge |z|^2 \quad \therefore |z| \le |x| + |y|$
...
\vspace{0.5em}
\textbf{（7）}
对于任意复数 $\lambda$，有： $\sum_{k=1}^n |z_k - \lambda w_k|^2 \ge 0$
...
\begin{tcolorbox}[title=定义：复数的辐角, colframe=blue!50!black, colback=blue!5!white]
...
\begin{tcolorbox}[title=无穷远点的四则运算规则, colframe=orange!75!black, colback=orange!5!white, sharp corners]
	关于 $\infty$ 的四则运算如下：
	\begin{itemize}
