\section{二维随机变量的分布}
\textbf{联合分布函数}：设 $(X, Y)$ 是二维随机变量，\textbf{联合分布函数} $F(x, y)$ 定义为：
\begin{equation}
	F(x, y) = P\{X \le x, Y \le y\}
	\label{eq:第三章:联合分布函数}
\end{equation}

\textbf{矩形区域概率公式}：对于 $x_1 < x_2, y_1 < y_2$，矩形区域概率为：
\begin{equation}
...
如果二维随机变量 $(X, Y)$ 所有可能取到的值是\textbf{有限对}或者\textbf{可列无限多对}，我们就称它为二维离散型随机变量。
...
\textbf{联合概率密度函数（$f(x, y)$）定义}：若存在非负函数 $f(x, y)$ 使得
\begin{equation}
...
\textbf{1. 二维均匀分布}：设 $D$ 是平面上的一个有界区域，其面积为 $S_D$。如果随机变量 $(X, Y)$ 的联合概率密度函数 $f(x, y)$ 为：
\begin{equation}
...
\textbf{2. 二维正态分布}：一个二维随机变量 $(X, Y)$ 服从均值为 $(\mu_1, \mu_2)$、方差为 $(\sigma_1^2, \sigma_2^2)$、相关系数为 $\rho$ 的正态分布，记作 $(X, Y) \sim N(\mu_1, \mu_2, \sigma_1^2, \sigma_2^2, \rho)$。其联合概率密度函数如下：
\begin{equation}
...
\subsection{边缘分布律}
\textbf{1. 离散型随机变量}：对于离散型随机变量 $(X, Y)$，其联合分布律为 $P(X=x_i, Y=y_j) = p_{ij}$。
\begin{itemize}
...
\textbf{形象理解}：如果你把联合概率分布写成一个表格，$X$ 的边缘分布就是每一行的\textbf{行和}，$Y$ 的边缘分布就是每一列的\textbf{列和}。这也是为什么它被称为"边缘"的原因——它们写在表格的边框处。

\textbf{2. 连续型随机变量}：对于连续型随机变量 $(X, Y)$，其联合概率密度函数为 $f(x, y)$。
\begin{itemize}
...
\section{随机变量的独立性}
\textbf{独立性（定义）}：$(X_1, \ldots, X_n)$ 和 $(Y_1, \ldots, Y_m)$ 相互独立，如果联合分布函数等于边缘分布函数的乘积：
\begin{equation}
...
\textbf{A. 和的分布：$Z = X + Y$}：这是最常见的情况。如果 $X$ 和 $Y$ 取值为整数，则：
\begin{equation}
...
\textbf{B. 最值分布：$M = \max(X, Y)$ 与 $N = \min(X, Y)$}
\begin{itemize}
...
\textbf{1. 分布函数法（通用且最稳妥）}：设 $Z = g(X, Y)$，步骤如下：
\begin{enumerate}
	\item 先求累积分布函数（CDF）：
	\begin{equation}
...
\textbf{2. 变量代换法（雅可比矩阵法）}：当你需要求从 $(X, Y)$ 到 $(U, V)$ 的联合分布变换时，此法非常高效。假设有变换 $u = g_1(x, y)$ 和 $v = g_2(x, y)$，且其逆变换 $x = h_1(u, v), y = h_2(u, v)$ 存在且一、二阶偏导连续，则 $(U, V)$ 的联合密度函数为：
\begin{equation}
...
\textbf{A. 和的分布：$Z = X + Y$}：如果 $X, Y$ 独立，其密度函数为两者的卷积：
\begin{equation}
	f_Z(z) = \int_{-\infty}^{+\infty} f_X(x)\,f_Y(z-x)\,dx
	\label{eq:第三章:卷积}
\end{equation}

\textbf{B. 商的分布：$Z = Y / X$}：若 $X, Y$ 独立且 $X > 0$，则：
\begin{equation}
