...
设 $\{X_n\}$ 为随机变量序列，$X$ 为随机变量，若任给 $\epsilon > 0$，使得
\begin{equation}
...
设 $\{X_k\}$ 为独立的随机变量序列，且有相同的期望 $E(X_k) = \mu$ 和方差 $\sigma^2 > 0$，则\textbf{样本均值} $\frac{1}{n}\sum_{k=1}^{n} X_k$ 依概率收敛于 $\mu$：
\begin{equation}
...
设 $\mu_n$ 是 $n$ 次伯努利试验中事件 $A$ 发生的次数，$p$ 是 $A$ 发生的概率，则\textbf{频率} $\frac{\mu_n}{n}$ 依概率收敛于\textbf{概率} $p$：
\begin{equation}
...
\section{中心极限定理}
\begin{theorem}[林德伯格-列维定理（独立同分布）]
设 $X_1, X_2, \ldots, X_n$ 相互独立，服从同一分布，且 $E(X_i) = \mu$，$D(X_i) = \sigma^2 > 0$。则当 $n$ 充分大时，\textbf{和的标准化变量}的分布函数趋于标准正态分布函数 $\Phi(y)$：
\begin{equation}
...
\begin{theorem}[棣莫弗-拉普拉斯定理（二项分布）]
设 $\mu_n \sim B(n, p)$，则当 $n$ 充分大时，\textbf{频数的标准化变量}的分布函数趋于标准正态分布函数 $\Phi(x)$：
\begin{equation}
...
设 $X$ 表示一年内死亡人数，$X \sim B(n, p)$，$n = 10000, p = 0.006$。保险公司利润 $Y = 10000 \times 12 - 1000 X$。
\begin{itemize}
