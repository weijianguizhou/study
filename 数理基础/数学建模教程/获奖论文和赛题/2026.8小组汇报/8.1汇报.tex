...
\begin{frame}
	...
	\end{tikzpicture}
	\end{center}
	这篇论文的骨架，就是我们课程里学的标准数模流程：从问题重述、建模求解，到检验、总结，一步没落。
\end{frame}
	...
	\begin{columns}[T]
		...
		\begin{block}{一眼AI的原因}
			\begin{itemize}
				...
				\item \textbf{结构刻板}：永远是``方法 + 对象''，年份、赛事、背景一概没有。
...
\begin{frame}
	...
	\begin{columns}[T]
		...
		\end{alertblock}
	\end{columns}
	\remark{跟你们说}{摘要就是评委第一次看你的论文，别让人家翻半天找不到你干了啥、算出了啥。这篇摘要的写法，照着套就行。}
\end{frame}
...
\begin{frame}
	...
	\begin{block}{逐句拆开看}
		...
		\end{itemize}
	\end{block}
	\begin{block}{下次你写论文，第一段可以直接套这个模板}
		\begin{center}
			\texttt{[研究对象]}是\texttt{[一句话意义]}。\texttt{[装置]}由\texttt{[部件A、B、C]}组成。\texttt{[激励]}施加\texttt{[作用]}，带动\texttt{[过程]}，从而\texttt{[输出/结果]}。
		\end{center}
	\end{block}
	\remark{一点想法}{论文开头最怕两种极端：要么一句话没有直接进问题（读者不知道你在干嘛），要么扯了一页背景还没说到研究对象上。这篇的处理刚好——三句话，每句一个功能，不贪多。}
\end{frame}
...
\begin{frame}
	...
	\end{block}
	\medskip
	\remark{赏析}{问题一先让建模型、算位移算速度，算是热身；问题二才求功率最大的阻尼系数，还分常系数和变系数两种情况，参数范围都给好了。我读的时候觉得他们把约束写得清清楚楚，我们照着做就行，这点做得挺清楚。}
\end{frame}
...
\begin{frame}
	...
	\end{block}
	\medskip
	\remarkg{赏析}{问题三难度明显上来了：又垂荡又纵摇，振子一边转一边滑，还冒出个旋转阻尼；计算要求也细到前40个周期、0.2秒一个点。问题四再把两个阻尼系数一起优化、给定频率。说实话我看到这儿有点压力，感觉让我们自己做，未必能想得这么全。}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[T]
		...
		\end{alertblock}
	\end{columns}
	\remark{跟你们说}{问题重述是你建模的"订单"——写清楚要什么、约束是什么，后面建模才不会偏。我原来就是直接抄题目，后来发现出题的写法和建模需要的写法不是一回事。}
\end{frame}
...
\begin{frame}
	...
	\end{tikzpicture}
	\end{center}
	\remarkg{赏析}{方法就那几样，来回用，这大概是这篇最厉害的地方。理解得浅，说错的地方请大家指正。}
\end{frame}
...
\begin{frame}
	\frametitle{教程：写假设别凑条数}
	\scriptsize
	\begin{columns}[T]
		\column{0.5\textwidth}
		\begin{exampleblock}{要这样做}
			\begin{itemize}
				\item 每条假设要能在后面的模型里"对得上号"——省了哪个项、为什么能省
				\item 分两类：物理前提（线性波、理想流体）+ 简化取舍（忽略耦合、忽略质量摩擦）
				\item 写到后面能引用："基于假设X，方程中忽略XX项"
			\end{itemize}
		\end{exampleblock}
		\column{0.5\textwidth}
		\begin{alertblock}{不要这样做}
			\begin{itemize}
				\item 凑四条看起来整齐，但跟后面建模毫无关系
				\item 写的全部是"题目已给的已知条件"（波浪是简谐的）而不是你主动做的取舍
				\item "假设数据真实可靠"——这不是假设，是废话
			\end{itemize}
		\end{alertblock}
	\end{columns}
	\remark{跟你们说}{假设不是"题目让你写几条我就编几条"，是你建模时"为了简化放弃了什么"。这篇四条假设，后面方程都有一一对应，这才是好假设。}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[T]
		...
		\end{center}
	\end{columns}
	\remarkg{我的想法}{以前我总是一上来就写方程，写一半才发现漏力、方向反，返工很费时间。跟着论文一步步来，才明白``先受力分析再列方程''最省事。}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[T]
		...
		\end{center}
		\remark{我的想法}{二阶转一阶是标准动作，但我以前只停留在``会用''，没想过步长为什么要取这么小。自己跑一次才发现：步长一大，40 个周期算到后面就抖得没法看，$h=10^{-4}$ 才稳得下来。}
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\end{center}
		\column{0.5\textwidth}
		\remarkp{我的想法}{这个改参考系的小操作，我这次才算真会用了。}
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\end{center}
		\column{0.5\textwidth}
		\remark{我的想法}{变阻尼（$C=10000\,|v_r|^{0.5}$）和常数阻尼比，相对运动的形态明显不同。我这才体会到：阻尼模型不是随便选的，它直接决定运动结果。题目特意把两种情况分开让我们算，大概就是这个用意。}
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\end{center}
		\column{0.5\textwidth}
		\remarkg{我的想法}{看到单峰我是服气的：阻尼太小耗能不足、太大锁死运动，中间必有最优。我复现 $C\approx37186$、$P\approx229.77$W，和论文基本一致。}
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\end{center}
		\column{0.5\textwidth}
		\remarkp{我的想法}{两个参数是``一起''影响功率的，不能拆开各自求最优。我复现的峰位和论文略有出入，但``存在最优区域''的结论一致。}
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[T]
		...
		\end{center}
	\end{columns}
	\remark{我的想法}{问题三多了纵摇，第一步居然是算转动惯量。平行移轴、垂直轴定理我上课学过但从没用过，这次才发现它们是``组合体''求惯量的标准工具。坦白讲这一页我是对着课本啃下来的。}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[T]
		...
		\end{center}
	\end{columns}
	\remarkg{我的想法}{多自由度的题其实就是单自由度复制几份再串起来，会一个就差不多会一串了。}
\end{frame}
...
\begin{frame}
	\frametitle{下次写论文，你直接抄这个清单}
	\scriptsize
	\begin{block}{各部分的"写作公式"}
		\begin{itemize}
			\item \textbf{标题}：对象 + 任务，不用"基于……的研究"
			\item \textbf{摘要}：总述 $\rightarrow$ 每问（模型+结果数字）$\rightarrow$ 关键词三类
			\item \textbf{重述}：背景（意义）$\rightarrow$ 结构（部件）$\rightarrow$ 原理（链条）$\rightarrow$ 每问（含参数范围）
			\item \textbf{分析}：每问 = 承上基础 + 难点 + 对策 + 方法名，统一亮工具箱
			\item \textbf{假设}：每条 = 简化了啥 $\rightarrow$ 省了哪一项 $\rightarrow$ 方程里对得上
			\item \textbf{建模}：坐标/受力 $\rightarrow$ 方程 $\rightarrow$ 数值方法（步长/参数）$\rightarrow$ 结果（图+表+具体数）
			\item \textbf{检验}：验什么、怎么验、阈值多少——三个数字不能少
		\end{itemize}
	\end{block}
	\remark{跟你们说}{这就是读完这篇我总结的"模板清单"，下次建模论文照着这个骨架填内容，基本不会跑偏。当然具体怎么填还是要自己动脑，清单只是帮你记住格式。}
\end{frame}
