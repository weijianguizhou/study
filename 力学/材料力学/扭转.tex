...
\begin{align*}
    ...
    M_e &= \frac{60000 P}{2\pi n} \approx 9549 \frac{P}{n} \quad (\text{N}\cdot\text{m})
\end{align*}
...
\begin{itemize}
    \item 扭转时：圆周线大小、间距、形状不变；纵向线倾斜同一角度。
    \item 切应力公式：$\tau = \frac{T}{2\pi r_0^2 t}$
\end{itemize}

\section{切应力互等定理 (Theorem of Conjugate Shear Stress)}

\subsection{定义与表述}
在相互垂直的两个平面上，切应力必然成对出现，且数值相等、方向垂直于两平面的交线。其方向表现为：两个切应力要么同时指向交线，要么同时背离交线。

\subsection{数学表达式}
若在微体的 $x$ 面上存在切应力 $\tau_{xy}$，在与之垂直的 $y$ 面上必存在切应力 $\tau_{yx}$，且满足：
\begin{equation}
    \tau_{xy} = \tau_{yx}
\end{equation}
...
\subsection{变形公式}
\begin{itemize}
    \item \textbf{单位长度扭转角：} $\theta = \frac{d\varphi}{dx} = \frac{T}{G I_p}$
