...
\begin{tabular}{|c|c|c|}
\hline
\textbf{约束形式} & \textbf{长度因数 \(\mu\)} & \textbf{失稳时挠曲线形状特征} \\ \hline
两端铰支 & \(1.0\) & 正弦半波 \\ \hline
一端固定、一端自由 & \(2.0\) & 正弦四分之一波 \\ \hline
一端固定、一端铰支 & \(0.7\) & 正弦0.7波 \\ \hline
两端固定 & \(0.5\) & 正弦半波（两端有拐点） \\ \hline
\end{tabular}
\end{center}

\begin{myTheorem}{临界应力与柔度}
    压杆的柔度（长细比）定义为：
    \[
...
\begin{mydefinition}{直线公式（中柔度杆）}
    对于中柔度杆（\(\lambda_c \le \lambda < \lambda_p\)，\(\lambda_c\) 为材料常数），临界应力与柔度成线性关系：
    \[
    \sigma_{\text{cr}} = a - b\lambda
    \]
    式中 \(a, b\) 为与材料有关的常数，可通过实验测定。例如 Q235钢（\(\lambda_c \approx 60\)）：\(a = 304 \,\text{MPa}\)，\(b = 1.12 \,\text{MPa}\)。
\end{mydefinition}

\begin{mydefinition}{抛物线公式}
    某些规范（如我国钢结构设计规范）推荐采用抛物线公式：
    \[
    \sigma_{\text{cr}} = \sigma_s \left[ 1 - \alpha \left( \frac{\lambda}{\lambda_c} \right)^2 \right]
    \]
    其中 \(\sigma_s\) 为屈服极限，\(\alpha\) 为系数（通常取 0.43 左右），\(\lambda_c\) 为抛物线公式适用的上限柔度。
\end{mydefinition}

对于小柔度杆（\(\lambda < \lambda_c\)），压杆破坏主要是强度问题，取：
\[
\sigma_{\text{cr}} = \sigma_s \quad \text{（塑性材料）} \quad \text{或} \quad \sigma_{\text{cr}} = \sigma_b \quad \text{（脆性材料）}
\]

\begin{myhypothesis}{压杆分类汇总}
    \begin{itemize}
        \item \textbf{大柔度杆（\(\lambda \ge \lambda_p\)）}：弹性失稳，用欧拉公式。
        \item \textbf{中柔度杆（\(\lambda_c \le \lambda < \lambda_p\)）}：弹塑性失稳，用直线公式或抛物线公式。
        \item \textbf{小柔度杆（\(\lambda < \lambda_c\)）}：强度破坏，按压缩强度计算。
    \end{itemize}
\end{myhypothesis}
...
为保证压杆安全工作，必须满足稳定条件：
\[
\frac{F_{\text{cr}}}{F} \ge n_{\text{st}} \quad \text{或} \quad \frac{\sigma_{\text{cr}}}{\sigma} \ge n_{\text{st}}
\]
其中 \(n_{\text{st}}\) 为规定的稳定安全系数，一般大于强度安全系数（\(n_{\text{st}} = 1.5 \sim 3.0\)）。

工程上常采用\textbf{折减系数法}：
\[
\sigma = \frac{F}{A} \le \varphi(\lambda) \, [\sigma]
\]
式中 \(\varphi(\lambda)\) 为稳定系数（小于1），可从规范中查得；\([\sigma]\) 为材料的许用压应力。

\begin{myformula}
    \begin{aligned}
        &\text{稳定条件（安全系数法）：} & n &= \frac{F_{\text{cr}}}{F} \ge n_{\text{st}} \\
        &\text{稳定条件（折减系数法）：} & \sigma &= \frac{F}{A} \le \varphi(\lambda) \,[\sigma]
    \end{aligned}
\end{myformula}

\section*{典型例题}

\begin{example}{两端铰支钢压杆的临界压力}
...
比例极限柔度：
\[
\lambda_p = \pi \sqrt{\frac{E}{\sigma_p}} = \pi \sqrt{\frac{206\times10^9}{200\times10^6}} \approx 100.8
\]
因 \(\lambda > \lambda_p\)，故欧拉公式适用。
\end{solution}

\begin{example}{一端固定一端自由压杆的稳定校核}
...
\begin{remark}
    压杆稳定性设计时，应尽量使杆在两个形心主惯性平面内的柔度相等（等稳定性），以充分利用材料。例如采用工字形或空心截面时，应使 \(I_{\max}\) 和 \(I_{\min}\) 尽可能接近。
\end{remark}
