...
\begin{document}
	\begin{titlepage}
		\begin{tikzpicture}[remember picture, overlay]
			...
			\node[anchor=north] at (current page.north) {
				\includegraphics[width=\paperwidth]{Figure/云.jpg}
			};
			...
			\node[anchor=center, text=DarkBlue] at ([yshift=-20cm]current page.north) {
				\begin{minipage}{0.8\paperwidth}
					\centering
					{\fontsize{40}{48}\sffamily\bfseries Spacecraft Dynamics笔记} \\[1cm]
		...
		\end{tikzpicture}
	\end{titlepage}
	你魏哥看老师给的那个文档发现上个暑假学的都忘了。笔记也找不到了，气死了，遂重新整理一下，费点时间就费点时间吧。
	\newpage
	...
	\newpage
	\input{KINEMATICS}
    \input{Gravitational Forces}
    \input{Simple Spacecraft}
    \input{Complex Spacecraft}
\end{document}