...
\begin{myformula}
\textbf{轨迹概率}
$$P(\tau \mid \boldsymbol{\theta}) = p_0(S_0) \prod_{t=0}^{T-1} \pi_{\boldsymbol{\theta}}(A_t \mid S_t) \cdot \trans(S_{t+1} \mid S_t, A_t)$$
\end{myformula}
...
其中 $b(s)$ 是任意\textbf{仅依赖于状态}（不依赖于动作）的函数。
\end{myformula}
