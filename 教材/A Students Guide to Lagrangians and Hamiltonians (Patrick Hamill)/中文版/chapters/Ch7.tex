...
于是拉格朗日方程为
$$
\frac{\partial}{\partial t}\left(\frac{\partial\mathcal{L}}{\partial(\partial w/\partial t)}\right) + \sum_{i=1}^{3}\frac{\partial}{\partial x_i}\left(\frac{\partial\mathcal{L}}{\partial(\partial w/\partial x_i)}\right) - \frac{\partial\mathcal{L}}{\partial w} = 0, \eqno\hbox{(7.5)}
$$
或更紧凑地写成
$$
...
\caption{图 7.2　质量为 $m$ 的珠子穿在一根细而硬的金属丝上，由劲度系数为 $k$ 的弹簧相连。平衡间距为 $a$。下图显示了两颗珠子偏离平衡的距离分别为 $x_i$ 和 $x_{i+1}$。}
\end{figure}
...
$$
L = T - V = \sum_i\left[\frac{m}{2}\dot{x}_i^2 - \frac{k}{2}(x_{i+1} - x_i)^2\right]. \eqno\hbox{(7.9)}
$$
...
$$
当 $a \to 0$ 时，$m/a \to \lambda(x)$，即单位长度的质量，也就是杆在位置 $x$ 处的线质量密度。项 $(x_{i+1} - x_i)/a$ 也可以写成连续函数。令 $x_i = x(x)$，$x_{i+1} = x(x + a)$。为方便起见，我们用 $\Delta x$ 表示 $a$。于是
$$
\frac{x_{i+1} - x_i}{a} \to \frac{x(x + \Delta x) - x(x)}{\Delta x},
$$
在极限 $a \to 0$ 下，
$$
\frac{x_{i+1} - x_i}{a} \to \frac{dx}{dx},
$$
并且
$$
ka\left(\frac{x_{i+1} - x_i}{a}\right)^2 \to K\left(\frac{\partial x}{\partial x}\right)^2.
$$
求和变成对 $x$ 的积分，而
$$
L = \frac{1}{2}\int\left[\lambda(x)\left(\frac{\partial x}{\partial t}\right)^2 - K\left(\frac{\partial x}{\partial x}\right)^2\right]dx. \eqno\hbox{(7.10)}
$$
...
$$
其中 $x(x, t, 0)$ 是"真实"路径，$\epsilon$ 是一个小参数，$\eta(x, t)$ 是在端点处为零的任意函数。即
$$
\eta(x_1, t_1) = 0,
$$
以及
...
从而有
$$
0 = \left.\frac{dI}{d\epsilon}\right|_{\epsilon=0} = \int_{t_1}^{t_2}dt\int_{x_1}^{x_2}dx\left[\frac{\partial\mathcal{L}}{\partial x} - \frac{\partial}{\partial t}\left(\frac{\partial\mathcal{L}}{\partial\dot{x}}\right) - \frac{\partial}{\partial x}\left(\frac{\partial\mathcal{L}}{\partial x'}\right)\right]\left.\frac{\partial x}{\partial\epsilon}\right|_{\epsilon=0}.
$$
因此，拉格朗日方程为
$$
\frac{\partial}{\partial t}\left(\frac{\partial\mathcal{L}}{\partial\dot{x}}\right) + \frac{\partial}{\partial x}\left(\frac{\partial\mathcal{L}}{\partial x'}\right) - \frac{\partial\mathcal{L}}{\partial x} = 0, \eqno\hbox{(7.11)}
$$
正如前面得到的（见方程（7.4））。
...
正如前面所指出的，我们可以容易地把上面导出的关系推广到三维。这需要把标量位移 $x$ 换成矢量位移 $\boldsymbol{x}$，其分量为 $x_i$，其中 $i = (1, 2, 3)$ 或 $(x, y, z)$。量 $x$ 表示在空间某个区域的每一点都有定义的物理量，所以它满足场的定义。事实上，我们可以让 $x$ 表示诸如压强、速度或电磁场等其他物理量。
...
该方程的第 $i$ 个分量为
$$
\frac{\partial}{\partial t}\left(\frac{\partial\mathcal{L}}{\partial(\partial x_i/\partial t)}\right) + \sum_{k=1}^{3}\frac{\partial}{\partial x_k}\left(\frac{\partial\mathcal{L}}{\partial(\partial x_i/\partial x_k)}\right) - \frac{\partial\mathcal{L}}{\partial x_i} = 0, \eqno\hbox{(7.12)}
$$
...
$$
\frac{\partial}{\partial t}\left(\frac{\partial\mathcal{L}}{\partial\dot{x}_i}\right) + \sum_{k=1}^{3}\frac{\partial}{\partial x_k}\left(\frac{\partial\mathcal{L}}{\partial x'_{ik}}\right) - \frac{\partial\mathcal{L}}{\partial x_i} = 0. \eqno\hbox{(7.13)}
$$
...
因此，用泛函导数表示，哈密顿原理
$$
0 = \delta L = \int\left[\frac{\partial\mathcal{L}}{\partial x_i}\delta x_i - \sum_{k=1}^{3}\frac{d}{dx_k}\frac{\partial\mathcal{L}}{\partial(\partial x_i/\partial x_k)} + \frac{\partial\mathcal{L}}{\partial\dot{x}_i}\delta\dot{x}_i\right]d^3x
$$
可以表示为
$$
\delta L = \int\left[\frac{\delta\mathcal{L}}{\delta x_i}\delta x_i + \frac{\partial\mathcal{L}}{\partial\dot{x}_i}\delta\dot{x}_i\right]d^3x.
$$
...
\begin{exercise}
证明 $\dfrac{\delta\mathcal{L}}{\delta\dot{x}} = \dfrac{\partial\mathcal{L}}{\partial\dot{x}}$。
\end{exercise}
