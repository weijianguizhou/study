...
$$
函数 $\eta(x)$ 是任意的（端点处除外），所以式 (2.8) 成立当且仅当对所有 $x$ 的值都有 $f(x) = 0$。你可能会认为，要求所有 $x$ 都满足 $f(x) = 0$ 过于苛刻，但很容易说明事实并非如此。你可以这样理解：假定 $\eta(x)$ 除在 $x_i$ 与 $x_f$ 之间的某个点（比如 $x = \xi$）外处处为零。然后考虑积分
$$
\int_{x_f = \xi + \varepsilon}^{x_i = \xi - \varepsilon} f(x)\eta(x)\, dx = 0,
$$
其中 $\varepsilon$ 是一个很小的量。由于积分处处为零，它在上述积分区间内也为零。此外，$f(x)$ 在这个小区间上基本恒定。把它记为 $f(\xi)$，并移到积分号外：
$$
f(\xi)\int_{x_f = \xi + \varepsilon}^{x_i = \xi - \varepsilon} \eta(x)\, dx = 0.
$$
积分不为零，所以 $f(\xi)$ 必为零。但点 $x = \xi$ 可以位于 $x_i$ 到 $x_f$ 之间的任何地方，因此沿路径所有点处 $f(x)$ 均为零。
