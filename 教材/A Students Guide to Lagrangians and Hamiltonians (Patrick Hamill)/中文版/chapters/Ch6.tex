...
$$
H + \frac{\partial F}{\partial t} = 0. \eqno\hbox{(6.2)}
$$
...
由方程（5.14）
$$
p_i = \frac{\partial S}{\partial q_i}, \eqno\hbox{(6.3)}
$$
$$
...
$$
H\!\left(q_1, \ldots, q_n; \frac{\partial S}{\partial q_1}, \ldots, \frac{\partial S}{\partial q_n}; t\right) + \frac{\partial S}{\partial t} = 0. \eqno\hbox{(6.4)}
$$
...
由方程（6.3）
$$
p_i = \frac{\partial S}{\partial q_i} = \frac{\partial S(q_i, \alpha_i, t)}{\partial q_i}, \eqno\hbox{(6.5)}
$$
以及
$$
\beta_i = Q_i = \frac{\partial S}{\partial \alpha_i} = \frac{\partial S(q_i, \alpha_i, t)}{\partial \alpha_i}. \eqno\hbox{(6.6)}
$$
如果方程（6.6）可以求逆，我们便得到
$$
q_i = q_i(\alpha_i, \beta_i, t). \eqno\hbox{(6.7)}
$$
将（6.7）代入（6.5），得到
$$
...
即
$$
p_i = p_i(\alpha_i, \beta_i, t). \eqno\hbox{(6.8)}
$$
...
以及
$$
\frac{1}{2m}\left(\frac{dW}{dq}\right)^2 + \frac{1}{2}m\omega^2q^2 = \alpha. \eqno\hbox{(6.9)}
$$
...
所以
$$
q = \sqrt{\frac{2\alpha}{m\omega^2}}\,\sin\omega(t + \beta). \eqno\hbox{(6.10)}
$$
类似地，由（6.8）
$$
...
即
$$
p = \sqrt{2m\alpha}\,\cos\omega(t + \beta). \eqno\hbox{(6.11)}
$$
...
即
$$
S = \int L\,dt + \text{const.} \eqno\hbox{(6.12)}
$$
...
$$
\psi = \psi_0 e^{iS/\hbar}, \eqno\hbox{(6.14)}
$$
其中 $\hbar = h/2\pi$ 使指数成为无量纲量。

现在 $S$ 满足哈密顿-雅可比方程（6.4），我们可以把它写成
$$
\frac{\partial S}{\partial t} + H = \frac{\partial S}{\partial t} + \frac{p^2}{2m} + V = \frac{\partial S}{\partial t} + \frac{1}{2m}\left(\frac{\partial S}{\partial q}\right)^2 + V = 0. \eqno\hbox{(6.15)}
$$
...
所以
$$
\frac{\partial S}{\partial q} = -\frac{i\hbar}{\psi}\frac{\partial\psi}{\partial q}. \eqno\hbox{(6.16)}
$$
此外，由（6.2）
$$
...
所以（6.15）变为
$$
-E + \frac{1}{2m}\left(-\frac{i\hbar}{\psi}\frac{\partial\psi}{\partial q}\right)\left(-\frac{i\hbar}{\psi}\frac{\partial\psi}{\partial q}\right)^{*} + V = 0, \eqno\hbox{(6.17)}
$$
其中在计算平方时我们使用了复共轭，因为 $\psi$ 是复量。

重新整理方程（6.17），我们得到
$$
(V - E)\psi^{*}\psi + \frac{\hbar^2}{2m}\frac{\partial\psi^{*}}{\partial q}\frac{\partial\psi}{\partial q} = 0. \eqno\hbox{(6.18)}
$$
为方便起见，让我们定义
$$
...
两个固定电荷相距 $2d$。设 $r_1$ 和 $r_2$ 分别是从 $q$ 到 $q_1$ 和 $q_2$ 的距离。从柱坐标 $(\rho, \phi, z)$ 变换到由下式定义的椭圆坐标 $(x, \eta, \phi)$
$$
\rho = d\left[(x^2 - 1)(1 - \eta^2)\right]^{1/2}
$$
$$
\phi = \phi
$$
