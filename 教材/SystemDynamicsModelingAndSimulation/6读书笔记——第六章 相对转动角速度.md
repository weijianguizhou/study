...
![[SystemDynamicsModelingAndSimulation_63.png]]
...
若点 $P$ **固结在动系 $j$ 中**，则 $\boldsymbol{r}_j$ 为常列阵，$\dot{\boldsymbol{r}}_j=\boldsymbol{0}$，于是
...
## 6.3 方向余弦矩阵的时间变化率（$\boldsymbol{I}_3=\boldsymbol{A}_{ij}\boldsymbol{A}_{ij}^{\mathrm{T}}$）

> [!theorem] $\dot{\boldsymbol{A}}_{ij}$ 的左乘分解
...
## 6.4 方向余弦矩阵的时间变化率（$\boldsymbol{I}_3=\boldsymbol{A}_{ij}^{\mathrm{T}}\boldsymbol{A}_{ij}$）

> [!theorem] $\dot{\boldsymbol{A}}_{ij}$ 的右乘分解
...
> [!theorem] 相对转动角速度
> 结合方向余弦矩阵的坐标变换性质可知，$\tilde{\boldsymbol{b}}$ 与 $\tilde{\boldsymbol{d}}$ 应为某一个与坐标系 $i$ 和坐标系 $j$ 相对方位有关的矢量在 $i$ 系和 $j$ 系中的投影列阵的叉乘矩阵。该矢量记为 $\vec{\omega}_{ij}$，称为 $j$ 系相对于 $i$ 系的**相对转动角速度**：
>
> $$
...
## 6.6 方向余弦矩阵的时间变化率 $\dot{\boldsymbol{A}}_{ij}$（相对速度）
...
![[SystemDynamicsModelingAndSimulation_65a.png]]

*固结在动系上的点的相对速度（坐标系示意）*

**固结在动系上的点的相对速度列阵**：由 6.1 节，结合 $\dot{\boldsymbol{A}}_{ij}=\tilde{\boldsymbol{\omega}}_{ij|i}\boldsymbol{A}_{ij}=\boldsymbol{A}_{ij}\tilde{\boldsymbol{\omega}}_{ij|j}$，得
...
![[SystemDynamicsModelingAndSimulation_65b.png]]

*相对速度矢量 $\vec{v}$ 与 $\vec{\omega}_{ij}$、$\vec{r}$ 的几何关系*
...
>
> 由此可知，$j$ 系相对于 $i$ 系作**瞬时定轴转动**，瞬时转轴沿 $\vec{\omega}_{ij}$ 方向，相对转动角速度为 $\vec{\omega}_{ij}$。

## 6.7 科里奥利公式

![[SystemDynamicsModelingAndSimulation_66.png]]

*科里奥利公式推导用坐标系*

设 $\vec{r}$ 为**任意**矢量（不要求固结于动系），其坐标列阵满足
...
![[SystemDynamicsModelingAndSimulation_67.png]]
