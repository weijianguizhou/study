...
\begin{enumerate}
	\item \textbf{$a_{i-1}$（连杆长度）}：这是公垂线的长度，恒为非负。如果两轴平行，可在公垂线族中任选一条，通常选与前一公垂线相交的那一条。如果两轴相交，$a_{i-1}=0$。
	
	\item \textbf{$\alpha_{i-1}$（连杆扭角）}：描述了关节轴 $i$ 相对于关节轴 $i-1$ 的空间倾斜程度。借助右手定则：右手拇指指向 $x_{i-1}$，四指弯曲方向为 $\alpha_{i-1}$ 的正方向。
	
	\item \textbf{$d_i$（连杆偏距）}：对于\textbf{移动关节}，$d_i$ 是关节变量；对于\textbf{旋转关节}，$d_i$ 是常数。
	
	\item \textbf{$\theta_i$（关节转角）}：对于\textbf{旋转关节}，$\theta_i$ 是关节变量；对于\textbf{移动关节}，$\theta_i$ 是常数。
\end{enumerate}
