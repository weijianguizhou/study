...
\section{数学模型的线性化}
实际的物理系统中往往存在非线性项。而为了使用叠加原理对数学模型进行求解，往往需要进行线性化。

线性化方法通常是使用\textit{Taylor}展开取一次项。
...
\begin{keybox}[常用等效变换法则]
    \begin{itemize}
        ...
        \item \textbf{分支点后移}（由方框输入端移到输出端）：引出支路中串入 $1/G(s)$；
        \item \textbf{分支点前移}（由方框输出端移到输入端）：引出支路中串入 $G(s)$；
        \item \textbf{相加点后移}（由方框输入端移到输出端）：移到输出端的那条支路串入 $G(s)$；
        \item \textbf{相加点前移}（由方框输出端移到输入端）：移到输入端的那条支路串入 $1/G(s)$。
    \end{itemize}
\end{keybox}

上述四种移动规则的等效变换过程如图~\ref{fig:分支点移动}、图~\ref{fig:相加点移动} 所示。

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.46\textwidth}
        \centering
        \figplaceholder[0.42\textwidth]{分支点后移：引出点由输入端移到输出端，引出支路串入 $1/G(s)$}
        \caption{分支点后移}
        \label{fig:分支点后移}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.46\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/引出点前移.png}
        \caption{分支点前移}
        \label{fig:分支点前移}
    \end{subfigure}
    \caption{分支点（引出点）的移动规则}
    \label{fig:分支点移动}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.46\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/比较点后移.png}
        \caption{相加点后移}
        \label{fig:相加点后移}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.46\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/比较点前移.png}
        \caption{相加点前移}
        \label{fig:相加点前移}
    \end{subfigure}
    \caption{相加点（比较点）的移动规则}
    \label{fig:相加点移动}
\end{figure}
