...
\usepackage{bookmark}   % 接管 PDF 书签：消除 hyperref 的「书签锚点与父级相同」警告（目录含三级条目时必现）
...
% --- 图位占位：导入稿中尚未入库的截图，用虚线框标出待插图位置 ---
\newcommand{\figplaceholder}[2][0.7\textwidth]{%
	\begin{center}
		\begin{tikzpicture}
			\node[draw=gold, dashed, line width=0.7pt, rounded corners=4pt,
			      text width=#1, align=center, inner sep=7pt,
			      text=structurecolor!80, font=\small\kaishu]
			{【待插图】#2};
		\end{tikzpicture}
	\end{center}
}
...
% --- 以下封面配色仅供「封面存档/」中的备用封面使用，当前启用的是封面 cover_呐喊风.tex ---
%     呐喊风配色（sk* 前缀）定义在 cover_呐喊风.tex 内部，单独 \input{} 即可编译。
% --- 默认深蓝封面主题色（cover.tex 使用） ---
